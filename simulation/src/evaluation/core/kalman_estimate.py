...
class KalmanEstimate:
    """Estimated state output over time produced by a Kalman filter.

    All arrays share the same first dimension (number of samples).
    The coordinate frames and units match the reference GroundTruth of the simulation
    the estimate was produced from.
    """
    ...
    estimation_output_data_rate: float  # Hz
    covariance: np.ndarray | None = None  # shape (N, state_dim, state_dim)
    measurement_variances: dict[str, np.ndarray] = field(default_factory=dict)  # key -> (N, k)
    ...
    def validate(self) -> None:
        """Check kalman estimation data for obvious errors."""
        n = len(self.time)
        ...
        print("KalmanEstimate: validation passed!")
